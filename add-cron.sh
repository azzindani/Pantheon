#!/usr/bin/env bash
# Add the hourly briefing cron job for an agent. Runs the cron CLI as the hermes
...
#
# Schedule is fixed: minute <MIN>, hours 05-21, every day, Asia/Jakarta
...
docker exec -u 10000 "hermes-$NAME" "$H" cron create "$MIN 5-21 * * *" "$(cat "$PROMPT")" \
  --name briefing --deliver "telegram:$USERID"
chown -R 10000:10000 "$ROOT/agents/$NAME/data/cron" 2>/dev/null || true
echo "added briefing for $NAME at minute $MIN (hours 05-21, Asia/Jakarta), delivering to telegram:$USERID"
