#!/usr/bin/env bash
# Add the briefing cron job for an agent (every 2 hours). Runs the cron CLI as the hermes
...
#
# Schedule is fixed: minute <MIN>, hours 05,07,09,11,13,15,17,19,21 (every 2h), Asia/Jakarta
...
docker exec -u 10000 "hermes-$NAME" "$H" cron create "$MIN 5-21/2 * * *" "$(cat "$PROMPT")" \
  --name briefing --deliver "telegram:$USERID"
chown -R 10000:10000 "$ROOT/agents/$NAME/data/cron" 2>/dev/null || true
echo "added briefing for $NAME at minute $MIN (every 2h 05-21, Asia/Jakarta), delivering to telegram:$USERID"
